/*! @remino/jukette-audio v0.4.2 | (c) 2026 Rémino Rem <https://remino.net/> | ISC Licence */
...
var decodeId3TextFrame = (frameData) => {
	...
	if (encoding === 0) return (0, _remino_jukette_core.cleanMetadataText)((0, _remino_jukette_core.decodeIso88591)(content));
	if (encoding === 3) return (0, _remino_jukette_core.cleanMetadataText)((0, _remino_jukette_core.decodeTextBytes)(content, "utf-8"));
	if (encoding === 2) return (0, _remino_jukette_core.cleanMetadataText)((0, _remino_jukette_core.decodeUtf16Be)(content));
	return (0, _remino_jukette_core.cleanMetadataText)((0, _remino_jukette_core.decodeTextBytes)(content, "utf-16"));
};
var parseAudioFileMetadata = (buffer) => {
	const data = new Uint8Array(buffer);
	if (data.length < 10 || (0, _remino_jukette_core.decodeAscii)(data.slice(0, 3)) !== "ID3") return {};
	...
	while (offset + 10 <= tagEnd) {
		const frameId = (0, _remino_jukette_core.decodeAscii)(data.slice(offset, offset + 4));
...
var AudioPlayableTrack = class extends _remino_jukette_core.JukettePlayableTrack {
	audio;
